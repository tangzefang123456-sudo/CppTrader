...
struct MemoryPoolNode {
    T data;                    // 数据
    MemoryPoolNode* next;      // 下一个节点指针
    MemoryPoolNode* prev;      // 上一个节点指针
};
...
explicit MemoryPool(size_t initial_size = 1024);
    ...
    // 分配节点
    MemoryPoolNode<T>* Allocate();
    
    // 释放节点
    void Deallocate(MemoryPoolNode<T>* node);
...
int main() {
    // 测试内存池的基本功能
    common::MemoryPool<int> pool(10);
    
    std::cout << "初始大小: " << pool.Size() << std::endl;
    std::cout << "初始已分配大小: " << pool.AllocatedSize() << std::endl;
    
    // 分配第一个节点
    common::MemoryPoolNode<int>* node1 = pool.Allocate();
    std::cout << "分配第一个节点后已分配大小: " << pool.AllocatedSize() << std::endl;
    
    // 分配第二个节点
    common::MemoryPoolNode<int>* node2 = pool.Allocate();
    std::cout << "分配第二个节点后已分配大小: " << pool.AllocatedSize() << std::endl;
    ...
    std::cout << "释放第一个节点后已分配大小: " << pool.AllocatedSize() << std::endl;
    
    pool.Deallocate(node2);
    std::cout << "释放第二个节点后已分配大小: " << pool.AllocatedSize() << std::endl;
    
    // 重新分配节点
    common::MemoryPoolNode<int>* node3 = pool.Allocate();
    std::cout << "重新分配节点后已分配大小: " << pool.AllocatedSize() << std::endl;
    
    node3->data = 3;
    std::cout << "node3->data: " << node3->data << std::endl;
    
    // 测试内存池的自动扩容功能
    common::MemoryPool<int> pool2(5);
    
    std::cout << "\n初始大小: " << pool2.Size() << std::endl;
    
    // 分配6个节点，触发扩容
    common::MemoryPoolNode<int>* nodes[6];
    for (int i = 0; i < 6; ++i) {
        nodes[i] = pool2.Allocate();
        nodes[i]->data = i + 1;
    }
    
    std::cout << "扩容后大小: " << pool2.Size() << std::endl;
    std::cout << "扩容后已分配大小: " << pool2.AllocatedSize() << std::endl;
    
    // 释放所有节点
    for (int i = 0; i < 6; ++i) {
        pool2.Deallocate(nodes[i]);
    }
    
    std::cout << "释放所有节点后已分配大小: " << pool2.AllocatedSize() << std::endl;
